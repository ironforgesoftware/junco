...
| `assessCmd.ts`          | `junco assess <path\|owner/repo> [--auto-plan]` — resolve the target (path, or a watched `owner/repo`, including external/unowned entries), compose the machine-owned assess ticket (`buildAssessTicket`), and submit it; the daemon runs the actual audit via `assessFlow.ts`. Also `junco assess review [<id>]` (list/inspect the pending store, `assessReview.ts`) and `junco assess file <id> --all\|--only <fp,...>` (the human confirm step, `assessFiling.ts`).                                                                                                                                                                                                                                        |
