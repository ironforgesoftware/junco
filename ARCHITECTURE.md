...
| `reviewStore.ts`        | `makeReviewStore<T>(subdir)`: the generic durable pending-review queue factory lifted out of `assessReview.ts` — one JSON file per entry under `<state_dir>/<subdir>/` (atomic tmp+rename, slugified id→filename), never-throw reads (missing → empty/null, corrupt → skipped/error), archive-on-remove into a caller-named subdirectory. `assessReview.ts` and `commentReview.ts` are its two instantiations.                                                                                                                                                                                                                                                                                                |
