...
| File                    | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cli.ts`                | Entrypoint; subcommands `start`, `run-once`, `submit`, `dispatch`, `inbox-path`, `status`, `list`, `retry`, `outbox`, `prs`, `assess`, `analyze`, `doctor`, `dashboard`, `logs`, `schema`, `config`, `service`, `restart`, `auth`. A bare invocation with no config routes to `dashboard` (FTUE walkthrough); with a config, to `start`. Exposes testable `run(argv, deps)`.                                                                                                                                                                                                                                                                                                                                                          |
| `daemon.ts`             | `mainLoop`, `runScheduler` (worker.maxConcurrent > 1), `StopFlag` (+ forceSignal), `installSignalHandlers`, `sleepInterruptible`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `config.ts`             | `loadConfig`: zod-validated JSON → typed `Config`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `reviewStore.ts`        | `makeReviewStore<T>(subdir)`: the generic durable pending-review queue factory lifted out of `assessReview.ts` — one JSON file per entry under `<stateDir>/<subdir>/` (atomic tmp+rename, slugified id→filename), never-throw reads (missing → empty/null, corrupt → skipped/error), archive-on-remove into a caller-named subdirectory. `assessReview.ts` and `commentReview.ts` are its two instantiations.                                                                                                                                                                                                                                                                                                                         |
| `assessReview.ts`       | Durable pending-review store for `junco assess` — a thin wrapper over `reviewStore.ts` (`makeReviewStore<PendingAssess>("assess-review")`) preserving its original exports: `writePending`/`listPending`/`readPending`/`removePending` (archives to `assess-review/filed/` rather than deleting) / `pendingCount` (surfaced in `status`/`doctor`).                                                                                                                                                                                                                                                                                                                                                                                    |
| `commentReview.ts`      | Durable pending-review store for `junco analyze` — the second `reviewStore.ts` instantiation (`makeReviewStore<PendingComment>("comment-review")`): `writeDraft`/`listDrafts`/`readDraft`/`removeDraft` (archives to `comment-review/posted/` or `comment-review/discarded/`) / `draftCount` (surfaced in `status`/`doctor`). `composeCommentBody` is the one place draft + disclosure footer get concatenated, so post and preview render identically.                                                                                                                                                                                                                                                                               |
| `assessFiling.ts`       | Phase B of two-phase assess — the least-privilege filing core shared by the CLI `file` command (and future TUI confirm): re-runs authoritative author-scoped GitHub dedup on the selected findings, best-effort ensures owned-repo labels (falls back to label-free on a permission glitch rather than failing the issue), files each selected/un-filed finding through the outbox (`tryOrEnqueue` → live `gh issue create` or a queued `issue-create` op), then archives the batch. External batches are label-free by construction — the label set is computed from the batch, not branched in code.                                                                                                                                |
| `githubOutbox.ts`       | Offline store-and-forward for GitHub side effects: FIFO op files under `<stateDir>/github-outbox/`, `tryOrEnqueue` seam (try live, queue on network error), flush executor (checkpointed composite PR ops, comment idempotency markers, dead-letter after 3 attempts). Also owns the `issue-create` op (`junco assess` findings) — `fetchFindingMarkers` scans the operator's own issues (`--author @me --state all`, not label-filtered, so it dedups identically on owned and unowned repos) fresh at flush time before creating, so replays stay dedup-safe.                                                                                                                                                                       |
...
| `statusCmd.ts`          | `junco status` — daemon /health + queue counts at a glance, plus the pending assess-review count (`assessReview.ts`) and the pending analyze-draft count (`commentReview.ts`) when non-zero.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `doctor.ts`             | `junco doctor` — preflight config/toolchain/endpoint/model/dirs, plus the pending assess-review and analyze-draft counts when non-zero (informational, not a health problem).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
