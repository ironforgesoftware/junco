...
describe("runDoctor assess history checks", () => {
  ...
  it("reports per-repo assess history informationally — never as a warning", async () => {
    const stateDir = mkdtempSync(join(tmpdir(), "junco-doc-history-"));
    recordRun({ stateDir } as unknown as Config, "o/r", {
      ok: true,
  ...
  it("shows a failed last attempt as informational, not a warning — never assessed", async () => {
    const stateDir = mkdtempSync(join(tmpdir(), "junco-doc-history-fail-"));
    recordRun({ stateDir } as unknown as Config, "o/other", {
      ok: false,
      at: "2026-07-16T00:00:00.000Z",
      reason: "boom",
    });
