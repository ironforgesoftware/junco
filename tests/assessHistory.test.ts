...
describe("assessHistory", () => {
  ...
  it("a failure preserves the last success and stamps the failure fields", () => {
    ...
    recordRun(cfg(s), "o/r", { ok: false, at: "2026-07-16T00:00:00.000Z", error: "boom" });
    const h = readHistory(cfg(s), "o/r")!;
  ...
  it("a success clears a prior failure", () => {
    const s = sandbox();
    recordRun(cfg(s), "o/r", { ok: false, at: "2026-07-15T00:00:00.000Z", error: "boom" });
    recordRun(cfg(s), "o/r", {
      ok: true,
      at: "2026-07-16T00:00:00.000Z",
  ...
  it("keeps separate repos in separate files (no shared-map lost update)", () => {
    const s = sandbox();
    recordRun(cfg(s), "o/one", {
      ok: true,
      at: "2026-07-16T00:00:00.000Z",
      value: { found: 1, parked: 1 },
    });
    recordRun(cfg(s), "o/two", {
      ok: true,
      at: "2026-07-16T00:00:00.000Z",
      value: { found: 2, parked: 2 },
    });
