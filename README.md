...
The **queue card** at the bottom of the left rail shows the daemon's local queue at all times: the running ticket (with live turn progress from the daemon's health endpoint) and how many tickets are waiting. Press `t` for the full queue view — waiting positions match the order the daemon will actually claim (priority first, then filename), deferred tickets show their retry backoff (`not before HH:MM`), and RECENT lists the last few finished tickets. The card covers the *whole* local queue, including tickets submitted with `junco submit` — not just GitHub-dispatched ones. When the daemon is down the card says so rather than implying queued work will run.
