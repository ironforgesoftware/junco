...
| Decision        | Choice                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Focus keys      | Main view: `w` opens the add-repo form (the watchlist action; `A` stays as an alias); `i` jumps to the issues pane. REVISED after live feedback: pane focus proved invisible — `w` now owns the watchlist action instead.                                                                                                                                                                                                         |
| Palette         | `:` opens an overlay of CLI subcommands (name + USAGE description), typed prefix filter, `↑/↓` select (letters type into the filter), `enter` runs, `esc` closes. Arg-taking commands (`list`, `retry`, `submit`, `logs`, `service`) show a TextField args step first (whitespace-split argv — no shell, no injection surface).                                                                                                   |
