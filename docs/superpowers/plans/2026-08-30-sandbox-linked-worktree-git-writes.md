...
> **Amendment (2026-08-30):** after the whole-branch review the maintainer reversed the whole-`.git` ruling — the shipped grant is the minimal set (the gitdir plus the common dir's `objects/`, `refs/`, `logs/`, a missing `logs/` created first for bubblewrap). The task text below describes the original design; the code and docs are authoritative.
