...
export function spawnCli(sb: Sandbox, args: string[]): DaemonHandle {
  ...
  const handle: DaemonHandle = { child, timedOut: false, exited: Promise.resolve(null as unknown as CliResult) };
...
export async function runCli(sb: Sandbox, args: string[], opts: { timeoutMs?: number } = {}): Promise<CliResult> {
...
**Files:**
- Create: `tests/e2e/happyPath.ts` (the shared script — a plain module, NOT a test file: importing a `*.e2e.ts` file from another test file would make vitest collect its tests twice), `tests/e2e/prFlow.e2e.ts`
- Possibly modify: `tests/e2e/harness.ts` (`defaultGhCases`) — see Step 3.

**Interfaces:**
- Consumes: everything from Task 2's harness; `Turn` from Task 1.
- Produces: `HELLO`, `HAPPY_PATH_SCRIPT: Turn[]`, `HAPPY_PATH_BODY: string` from `tests/e2e/happyPath.ts` (Task 9 reuses them).

- [ ] **Step 1: Write the shared script and the scenario**

`tests/e2e/happyPath.ts`:
...
```bash
git add tests/e2e/happyPath.ts tests/e2e/prFlow.e2e.ts tests/e2e/harness.ts tests/e2e/stubModel.ts
...
**Interfaces:**
- Consumes: `spawnDaemon`, `waitFor` (Task 2); `HAPPY_PATH_SCRIPT` / `HAPPY_PATH_BODY` from `tests/e2e/happyPath.ts` (Task 4).
...
- **Names used across tasks:** `createSandbox`, `writeTicket`, `runOnce`, `runCli`, `spawnDaemon`, `waitFor`, `queueState`, `transcript`, `ghLog`, `stub`, `chatRequests`, `remote.{branches,log,show,ahead}`, `defaultGhCases`, `childEnv`, `binaryUnderTest`, `spawnCli`, `Sandbox`, `CliResult`, `DaemonHandle` (Task 2); `Turn`, `StubModel`, `RecordedRequest`, `encodeTurn`, `encodeCompletion`, `startStubModel` (Task 1); `HELLO`, `HAPPY_PATH_SCRIPT`, `HAPPY_PATH_BODY` in `tests/e2e/happyPath.ts` (Task 4, consumed by Task 9). Test names contain their spec slugs verbatim (`pr-happy-path`, `qa-read-only`, `transient-requeue`, `daemon-lifecycle`, `live-pr`) so `-t <slug>` selects one.
