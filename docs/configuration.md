...
Set `JUNCO_CONFIG` to point junco at a specific config file, bypassing the canonical/legacy resolution above entirely — it wins even when a real `~/.junco/config.json` exists, and even when the named file doesn't exist yet (so a script can name the config it's about to create). A leading `~` expands to the home directory; empty or whitespace-only is treated as unset; a relative value is resolved to an absolute path immediately, so every path derived from it (including the lock below) agrees no matter which directory a given command runs from — prefer an absolute value anyway, since a relative one is still read against the launch directory once. Config resolution never looks at argv. Because `worker.lock` (see below) is derived from the resolved config's directory, this also relocates the daemon-singleton lock: two processes started with two different `JUNCO_CONFIG` values take two different locks, and both run.

> **⚠️ Give a second config its own `dataDir`.** Two configs are two _instances_ only when they also resolve to two different data roots. `worker.lock` lives next to `config.json`, not under `dataDir` — so two configs in two directories that point at the **same** `dataDir` give you two daemons, each holding its own lock, working one queue. That corrupts it: each daemon's startup orphan recovery requeues (or, once the retry budget is spent, fails) the _other_ daemon's live in-flight ticket, its worktree prune removes the other's live worktrees, and both race on the same fixed-name `spend.json.tmp` / `metrics.json.tmp`. Nothing detects this — the only collision signal is the health-port bind, which is a non-fatal warning and happens after orphan recovery has already run. Set a distinct `dataDir` in every config you intend to run concurrently.
