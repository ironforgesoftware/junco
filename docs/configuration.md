...
| `chat.fastForward`           | `true`  | live     | Fast-forward a junco-**managed** clone to `origin/<default branch>` when a chat session opens (and after `/new`), so the chat reads the commit a ticket run would build on. `merge --ff-only` only — never a reset, never a branch switch — and never on a checkout junco does not manage (a watchlist entry pointing at your own working copy is read, never written). Skipped, with the reason on the transcript, when the tree is dirty, `HEAD` is detached or on another branch, `origin/HEAD` is unset, or there are local commits `origin` does not have; a failed fetch (bounded at 15s) opens the chat on the tree as it stands. Off still records which commit the chat read. |
...
| `chat.thinkTags`             | `auto`  | restart¹ | What to do with literal `<think>…</think>` spans an OpenAI-compatible endpoint streams inside the answer: `auto` splits them into the thinking block unless the turn already streams native reasoning (a separate `reasoning_content` field), in which case the text passes through untouched; `on` always splits; `off` never does — the tags stay in the answer. `junco doctor` prints an `ℹ chat thinking` hint naming the server-side flag that makes the split unnecessary (llama.cpp `--reasoning-format deepseek`, LM Studio "Reasoning → separate field").                                                                                                                     |
| `chat.theme`                 | `auto`  | restart  | Palette for the chat's highlighted code fences — Pi's built-in `dark` or `light` theme. `auto` lets the highlighter guess from the terminal background (`COLORFGBG`), which a light terminal that does not export it gets wrong; pin it here. Read by `junco dashboard` at start.                                                                                                                                                                                                                                                                                                                                                                                                      |
| `chat.maxFps`                | `60`    | restart  | Ceiling on the dashboard's paint rate while a chat turn streams (10–120): a burst of deltas is folded into one frame per tick, so this bounds repaints, not latency. Lower it (30) on a slow terminal, a small board, or over SSH, where 60 repaints a second cost more than they show. Read by `junco dashboard` at start — restart the dashboard, not the daemon.                                                                                                                                                                                                                                                                                                                    |
...
¹ `chat.thinkTags` is read once, when a chat session is built: a running session keeps the value
it started with, and a hot-reloaded value reaches only the sessions built after it — every
session after a daemon restart, or the one `/new` in the chat rebuilds without a restart.
...
Each time a chat session opens — the first prompt or attach after a daemon restart, a checkout
that moved, and the reopen after `/new` — the transcript gets one `checkout` row naming the
commit the conversation is reading, and whether junco advanced it (`chat.fastForward`). That row
is the answer to "was this drafted against the current tree?": `junco transcript --chat
<owner/repo>` shows it, and so does the chat pane itself.
