...
¹ `chat.thinkTags` is read when a chat session is built, so a change reaches sessions opened after
it (`/new` in the chat, or a daemon restart, rebuilds one); an already-open session keeps the
