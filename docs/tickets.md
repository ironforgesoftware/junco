...
| `workdir`         | path                | Q&A tickets only: the directory the read-only session runs in (defaults to the worker's processing directory; must sit under `git.allowedRepoRoots` when that list is non-empty, else the default is used with a warning).                                                                                                                                                                                                                                                                                                                         |
| `network`         | boolean             | Sandbox egress opt-in for this ticket's session; defaults to `sandbox.network` (deny). `true` widens this one ticket only.                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `audit`           | mapping             | Selects the audit flavor (see above). `{ auto_plan: bool, issue: number, issue_title: string }` — `issue`/`issue_title` are present only when scoped via `junco audit owner/repo#N`; `issue` steers the audit and the filed findings' `Context:` line, `issue_title` is self-documenting only (nothing reads it). Machine-composed by `junco audit` — don't set by hand. `assess:` is a permanently accepted legacy alias (same shape); when a ticket carries both, `audit:` wins. → [Repo audit guide](./audit.md)                                |
...
| `patch_no_amend`         | error    | An apply ticket cannot also carry `amends_pr` — amend mode never rebuilds the PR body, so a fallback would force-push undisclosed agent work.                                                                                                                  |

The prose rules (`steps_have_commits`, `files_table_referenced`, `files_paths_exist`, `no_cd_in_steps`) are skipped for apply tickets — a patch series has no Steps or Files table for them to check. `no_cd_in_verification`, `labels_exist`, and `github_request_scope` still run, and `no_forbidden_phrases` runs against the prose outside the fence.
...
- A `junco-patch` fence inside an amend (`amends_pr`) ticket is **refused**, not undefined: lint errors (`patch_no_amend`), and if lint is off, Phase 4 fails the ticket before apply mode or the agent runs (worktree preserved, routed to `failed/`).
...
| `tasks[].acceptance`   | yes      | Non-empty list; becomes the child's `## Behavior (acceptance — testable assertions)` list.                                                                           |
| `tasks[].prohibitions` | no       | Becomes the child's `## Prohibitions` list.                                                                                                                          |
| `tasks[].verification` | no       | Shell commands, one per line; emitted under the child's `## Verification (junco runs this — do NOT run it yourself)` heading as a bash block, which the worker runs. |
...
- **Backoff arithmetic.** `not_before` is `now + worker.retryBackoffSeconds × attempt` (default 60 s); a requeued file whose name would collide gains a `-r<n>` suffix; a ticket whose frontmatter is too malformed to hold the retry state is not requeued at all and goes straight to `failed/`.
...
- **Timeouts salvage work.** A session that hits its timeout after committing gets its commits pushed and a draft PR opened (status `timeout_partial`, routed to `done/` — the `done/` statuses are `completed`, `completed_no_changes`, `aborted_partial`, and `timeout_partial`) with a partial-run banner, instead of losing the work in a dead worktree.
