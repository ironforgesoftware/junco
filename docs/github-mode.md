...
**Authoring issues.** The loop above assumes a human (or another tool) writes the first issue. `junco assess <path|owner/repo>` lets Junco write it instead: it audits a repo — `npm audit` plus a read-only agent audit — and files one GitHub issue per finding, each carrying `junco:finding`. The trigger label isn't applied by default; pass `--auto-plan` to add it to every filed issue, feeding them straight into the dispatch → plan hop above on the bridge's next sweep (the target repo still has to be watched, same as any other issue). See the [vulnerability assessment guide](./assess.md).
...
**Etiquette invariant.** In fork-PR mode the **only** outward-facing writes Junco makes are: creating your fork, pushing the feature branch to that fork, and opening a draft PR upstream. It applies **no labels and posts no comments** on the upstream issue — an external repo is never added to the bridge's polling set, so nothing there is ever mutated on your behalf. The draft PR is the single artifact that reaches the upstream project, and it lands as a draft so a maintainer never sees work-in-progress as ready-to-merge.
...
- **Issue list:** when the issue list is served from cache (GitHub was offline during fetch), an `offline · HH:MM` badge in the issue pane's title row shows the cached-at timestamp, so you know the list is stale. Next to it, a `↻ 12s` freshness stamp shows how long ago that pane last completed a fetch — the cache's age while offline, or time since the last live poll otherwise (issues poll every 30s, PRs every 60s, `r` refreshes now); the same stamp appears in both the issues and PRs pane titles.
