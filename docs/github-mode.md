...
**When GitHub is unreachable** (network errors after retry backoff), Junco queues label operations, issue comments, and the PR push+create sequence as durable ops under the outbox (`<dataDir>/data/outbox/`, or `<dataDir>/outbox/` on a not-yet-migrated legacy tree — run `junco data` to see the resolved path) — one JSON file per operation. These ops are **stored durably**: if the daemon crashes, they survive the restart and are flushed automatically on the next bridge sweep once GitHub comes back online. Operations are **replayed in FIFO order** to preserve per-issue semantics, are **idempotent** (a crash mid-flush will not double-post comments or duplicate labels), and are **fault-tolerant**: a non-network error on a single op (e.g., you deleted a label) bumps its attempt count and dead-letters it after 3 attempts to the outbox's `dead/` subdir. The ticket itself still **finalizes done/failed** locally (the worker never blocks on GitHub connectivity), so the queue keeps moving — the outbox is for durability, not gating.
...
**Dead-letter and recovery:** when an op has failed 3 times, it moves to the outbox's `dead/` subdir to prevent infinite retry. `junco doctor` warns if dead-lettered ops exist; list them with `junco outbox` (shown as `dead: N`). To retry a dead-lettered op, edit it by hand and move it back up one level — run `junco data` first to get the outbox's actual resolved path (`<dataDir>/data/outbox/` on a fresh install; `<dataDir>/outbox/` on a not-yet-migrated legacy tree — moving it to the wrong one strands it) — then run `junco outbox flush`.
