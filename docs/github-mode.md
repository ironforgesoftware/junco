...
**Dead-letter and recovery:** when an op has failed 3 times, it moves to `github-outbox/dead/` to prevent infinite retry. `junco doctor` warns if dead-lettered ops exist; list them with `junco outbox` (shown as `dead: N`). To retry a dead-lettered op, edit it by hand, move it back to `<stateDir>/github-outbox/`, and run `junco outbox flush`.
