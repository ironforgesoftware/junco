 ...
 * navigate back + toast → best-effort bot-access grant. `aliveRef` drops any
 * continuation after unmount (five guards below — one per await) so an
 * in-flight validate never writes the watchlist post-unmount. `setAddRepoError`
...
export function useAddRepoForm(opts: {
  ...
  const handleAddRepo = useCallback(
    ...
    [client, watchlistError, clonesDir, showToast, addEntry, setView, aliveRef],
  );
