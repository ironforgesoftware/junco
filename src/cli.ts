...
import { runStartCommand, type StartCmdDeps } from "./startCmd.js";
import { runSubmitCommand, type SubmitCmdDeps } from "./submitCmd.js";
...
/** Every `start` seam (locks, signals, mainLoop, the config watcher, bot auth)
 *  is declared on `StartCmdDeps` in startCmd.ts and inherited here, so
 *  `run(['start'], deps)` can forward this object through unchanged. */
export interface CliDeps extends StartCmdDeps {
  ...
  submitPlanFn?: SubmitCmdDeps["submitPlanFn"];
...
//
// DELIBERATELY UNANNOTATED (#351): a `ReturnType<typeof parseArgs>` return type
// collapses `values` to a `{ [k: string]: … }` index signature, which is what
// forced every reader to write `values.x as string | undefined`. Inferring from
// the literal `options` object below types each flag exactly — and makes a
// mistyped flag name a compile error rather than a silent `undefined`.
// ---------------------------------------------------------------------------
...
function parseCli(argv: string[]) {
  return parseArgs({
    ...
    options: {
      ...
      // (takes a value — unlike --plan, which is a boolean). See submitCmd.ts
      // and composePatchTicket (patchTicket.ts).
      patch: { type: "string" },
      title: { type: "string" },
      why: { type: "string" },
      verify: { type: "string" },
...
export async function run(argv: string[], deps: CliDeps = {}): Promise<number> {
  ...
  // (`start`'s own seams — the three locks, signal handlers, mainLoop, the
  // config watcher — are resolved inside runStartCommand; `deps` reaches it
  // unchanged.)
  ...
  // ------------------------------------------------------------
  // Subcommand table (#351). Each entry is the whole body of what used to be an
  // `if (subcommand === …)` branch, in the same order; the closures capture the
  // resolved collaborators above and are only ever invoked one at a time.
  const handlers: Record<string, () => Promise<number>> = {
    service: async () => {
      ...
      const maxQueuedTimeoutSecondsFn =
        deps.maxQueuedTimeoutSecondsFn ?? scanMaxQueuedTimeoutSeconds;
    ...
    // ------------------------------------------------------------
    // start: the foreground daemon (src/startCmd.ts)
    // ------------------------------------------------------------
    start: async () => {
      // The daemon proper (startCmd.ts): the three single-instance claims,
      // signal handlers, the config watcher and the mainLoop handoff. `deps`
      // passes straight through — CliDeps extends StartCmdDeps, so every seam a
      // test injects into run() reaches it unchanged.
      return runStartCommand(configPath, { once: values.once }, deps);
    },
    ...
    retry: async () => {
      const cfg = loadConfigFn(configPath);
      return runRetryCommand(cfg, positionals.slice(1), { all: values.all }, { printFn });
    },
    ...
    unwatch: async () => {
      ...
      return runUnwatchCommandFn(cfg, positionals.slice(1), { plan: values.plan }, { printFn });
    },
    ...
    audit: async () => {
      ...
      if (sub === "file") {
        ...
        return runAssessFileCommand(
          cfg,
          positionals[2],
          { all: values.all === true, only: values.only },
          { printFn },
        );
      }
      if (sub === "discard") {
    ...
    logs: async () => {
      const cfg = loadConfigFn(configPath);
      const n = values.lines !== undefined ? parseInt(values.lines, 10) : undefined;
      return runLogsCommand(cfg, {
        ...
        json: values.human ? false : values.json || undefined,
      });
    },
    ...
    restart: async () => {
      ...
      return runRestartFn(configPath, { force: values.force });
    },
    ...
    data: async () => {
      ...
      return runData(cfg, configPath, { json: values.json }, { printFn });
    },
    ...
    submit: async () => {
      return runSubmitCommand(
        positionals.slice(1),
        {
          ...
          asIssue: values["as-issue"],
        },
        {
          loadCfg: () => loadConfigFn(configPath),
          printFn: deps.printFn,
          readStdinFn: deps.readStdinFn,
          runSubmitDryRunFn: deps.runSubmitDryRunFn,
          submitPlanFn: deps.submitPlanFn,
        },
      );
    },
  ...
  // ------------------------------------------------------------
  // Dispatch: ONE table lookup, so an unrecognized verb fails here instead of
  // falling through 29 string comparisons. Object.hasOwn gates it because a
  // plain object literal inherits Object.prototype — without the gate,
  // `handlers["toString"]` resolves to a real function and `junco toString`
  // would "succeed".
  // ------------------------------------------------------------
