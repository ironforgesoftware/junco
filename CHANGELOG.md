...
- Corrects a Stage-1 apply-tickets claim: apply runs are **not** transcript-less. `docs/tickets.md` previously said an apply ticket runs no agent session so `junco transcript`/the dashboard's transcript view have nothing to show for it — apply runs now write their own transcript frames (see the Added entry above), so that statement no longer holds. The two other apply-tickets defaults stated unconditionally above — critic skip, and a conflict always failing the ticket outright — now hold only for a clean apply with the escalation ladder's fallback lever left untriggered; see the Added entry above for the conditional behavior.
