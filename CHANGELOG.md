...
- End-to-end test layer (`npm run test:e2e`, run in the quality gate): the built CLI runs as a child process in a sandboxed HOME with the real agent SDK driven against a scripted OpenAI-compatible model stub — PR happy path, read-only Q&A, transient-failure requeue, and daemon lifecycle (start, `/health`, poll-driven drain, SIGTERM) — and `scripts/package-smoke.sh` now runs the PR scenario through the tarball-installed binary. `npm run test:e2e:live` runs the same harness against a real inference endpoint (`JUNCO_E2E_MODEL_ID` / `JUNCO_E2E_BASE_URL` / `JUNCO_E2E_API_KEY`).
- `SECURITY.md` gives junco a disclosure channel (#383): the supported version window, reporting through this repository's private vulnerability reporting with a 72-hour acknowledgement, and the four in-scope boundaries — the execution sandbox, the GitHub bridge's approval chain, bot-account auth, and the health endpoint — alongside an explicit out-of-scope list, so a reporter knows up front what will and will not be treated as a vulnerability. A pull-request template and an `.editorconfig` land beside it.
- `.nvmrc` pins the exact `engines` floor (22.19.0), so `nvm use` and anything else that reads it lands on the version CI treats as the floor, and the quality gate gained a non-blocking node 26 canary leg (#382). Node 26 becomes LTS in October 2026 while the gate otherwise exercises only 22.19 and 24; the canary annotates a run but is deliberately kept out of the aggregate `quality-gate` job, so it can never block a merge.
- Plan sets are documented, and two new ticket templates ship in the package. Plan sets landed in 0.12.0 behind `[planSets]`, but a reader of `docs/` could not find the section, its three levers, or the `junco-plan` fence they actually write (#375). [Configuration § Plan sets](docs/configuration.md#plan-sets) now covers the levers and the fact that the Layer-1 `depends_on` machinery stays on with the lever off, and [Tickets § Plan sets](docs/tickets.md#plan-sets-the-junco-plan-fence) covers the local and bridge doors, a minimal fence, the field table, and what the compiler builds and refuses. `examples/apply-ticket.md` and `examples/plan-set.md` join the shipped examples, checked against the real parsers by the suite so a compiler or lint change cannot break them silently.
- Every version heading in this file resolves to its tag diff (#374). The file declared Keep a Changelog conformance, but the `[x.y.z]:` reference block that resolves the headings never existed — so on npm and GitHub all sixteen rendered as literal bracket text. A release that adds a heading without its link now fails the gate.
- Per-repo chat with the coding agent inside the dashboard (`t` on a repo row): an ongoing, read-only conversation, file-backed and daemon-owned so it survives a dashboard quit or a daemon restart. Every dispatch branch is reachable from it — a plain ticket, a ticket set, a plan set, an amend on an open PR, an apply-ticket patch, or a request to `audit`/`investigate` — each parked for the same human confirm audit/investigate findings already use (`s` submit, `e` edit, `r` cycle route, `D` discard). New config block `chat.*` (`chat.enabled`, `chat.modelId`, `chat.thinkingLevel`, `chat.turnTimeoutMinutes`); new data-tree dirs `data/chats`, `data/chat-drafts`; new loopback-only `/chat/*` routes on the health server (`GET /chat/events`, `POST /chat/prompt`, `POST /chat/abort`, `POST /chat/new`, `POST /chat/note`, `GET /chat/status`); `junco transcript --chat <owner/repo>` prints a session's transcript outside the TUI.

### Changed

- Dashboard shortcut bar redesigned as two rows (spec `docs/superpowers/specs/2026-09-02-footer-redesign-design.md`): the actions row names the thing under the cursor and lists its verbs — the chat chip first, as a filled pill — and a stable navigate row keeps the structural keys in the same place in every view; the toast now paints over the actions row instead of occupying its own line. `?` opens help from every overlay and returns to it on any key. **Key changes:** `c` is chat on every screen with a repo in context — including the repo-detail overlay and the transcript and review views, which also carry the chat chip — and from an issue or PR it prefills `/issue N` / `/pr N`; `t` is the ticket transcript only (it no longer opens chat from the rail or the PR pane); the `c commands` chip is retired — the palette is `:`.
...
- The documentation caught up with the surface junco actually ships, and each correction is now pinned by a test so it cannot drift again. `docs/operations.md`'s self-described "full CLI reference" had stopped at the 0.7-era commands — about twenty subcommands and their flags were missing, and bare `junco` was described as an alias for `start` when it ensures the supervised daemon and then opens the dashboard — and is now regenerated from `junco --help`, which the section names as the authority (#371). The README documents the three `junco submit` routes beyond a plain file: `--patch` (apply tickets), `--plan` (plan sets) and `--as-issue` (the parked, bot-authored issue door) (#372). `CONTRIBUTING.md` quotes the real five-step gate CI enforces instead of `npm test` alone, the real suite size, the `feat/<topic>` branch convention, and the exact-pin dependency policy (#373). `ARCHITECTURE.md`'s module map gained rows for 23 `src/` modules it never listed — the whole setup wizard, the lever registry and hot-reload watcher, the provider gate, the spend ledger and the small shared primitives — and its `cli.ts` row now points at the `USAGE` string rather than repeating a verb list that had already drifted (#376). Two statements in it were simply wrong and are corrected: the execution sandbox is on by default, not opt-in (#370), and `junco audit`'s filing dedup is marker-based across the repository's most recent 500 issues — any author, closed included — not author-scoped (#378).
- **Dependency major:** zod 3.25.76 → 4.5.4 (#381). Junco imports zod in two source files, and three of v4's breaking changes land on the config schema. The one that mattered: `.default()` no longer parses what it returns, so every `z.object({…}).default({})` section would have handed back a literal `{}` and left the roughly 97 leaf defaults below it `undefined` — a default `config.json` would not have loaded at all. Sections now use v4's `.prefault({})`, which is the v3 behaviour exactly; `z.record()` calls name their key schema, and one `.refine()` moved from `{ message }` to `{ error }`. No validation was loosened and no schema semantics were rewritten. Lockfiles will see the major; the agent SDK's own transitive zod dependencies dedupe onto 4.5.4 with no nested v3 copy.
- Patch/minor dependency sweep, all exact-pinned: `@earendil-works/pi-coding-agent` 0.84.2 → 0.84.4, `ink` 7.1.1, `react` 19.2.8, `vitest` and `@vitest/coverage-v8` 4.1.11, `eslint` 10.9.1, `typescript-eslint` 8.69.0 (#380). The deliberate security-only Dependabot policy now has a cadence to go with it: a monthly workflow runs `npm outdated` and files one issue with the table, leaving the actual bump to go through the full gate by hand. `prettier` deliberately stays at 3.8.4 — 3.9.6 reformats sixteen files and would fail `format:check`.
- The published tarball is a third smaller: `dist/**/*.js.map` is excluded from the `files` allowlist (#379). Every map pointed its `sources` at `../src/`, which the package does not ship, so an installed copy carried 223 maps that resolved to nothing — 685 files → 462, 1.08 MB → 849 KB packed. The package also declares `"exports": {}`, making "bin-only CLI, no import surface" a stated contract: the installed binary is unaffected (npm links `bin` without consulting `exports`) and dispatchers keep consuming `junco schema` JSON, while bare and deep imports of `dist/` now fail with `ERR_PACKAGE_PATH_NOT_EXPORTED`.
- `junco restart` preflights a checkout-backed install (#384). When the running `dist/` resolves inside a git checkout, it now refuses — exit 1, before any unit scan or restart kick — if the working tree is dirty or `HEAD` is not `origin/main`, printing the checkout path, both commits, and up to ten dirty paths; `junco restart --force` restarts anyway with a warning. An ordinary npm install (no `.git` beside `dist/`), or a host where git cannot answer, is untouched. On the release side, the npm publish job now deploys through a reviewer-protected GitHub environment, so a release run parks for an explicit approval instead of publishing on a tag alone. Both are documented under [Operations § Releasing](docs/operations.md#releasing-maintainers).
- `junco data migrate`, and the pre-0.10 flat-layout / `vaultRoot` migration path it exists for, will be removed in 1.0 (#360). Nothing changes before then and nothing is ever deleted by the migration — but an operator on an older install should migrate on any 0.x release, because from 1.0 on nothing in junco relocates an unmigrated root. See [Configuration § `junco data migrate`](docs/configuration.md#junco-data-migrate--the-opt-in-full-unification).
- Internal structure: the 2026-09-01 repository sweep (#387) reorganized the runtime without changing what it does — `submit` and `start` moved out of `cli.ts` into their own modules behind a dispatch table, the PR flow's Phase 9 into `postSessionReview.ts`, `runDoctor`'s 25 checks into a check table, the dashboard's 458-line action memo into three hooks, `config.ts` into paths/schema/assembly, and one writer, one `Result` type, one set of shared run/error helpers and one set of `gh` timeout budgets replaced the hand-copied versions scattered across modules. Comment cross-references now cite symbol names rather than line numbers that had drifted, git invocations terminate their options with `--` before positional refs and paths, `no-explicit-any` and a 400-line function ceiling are enforced by the gate instead of by review, the coverage floor was ratcheted and given per-tree floors for the sandbox and session modules, and the test suite's residual flake surface and duplicated harness helpers are gone. Three user-visible consequences ride along: `junco skill install --harness <dir>` now refuses to write over a `config.json` that is already schema-invalid elsewhere (exit 1, file untouched) instead of writing it and then dying at the next load; a save failure in the dashboard's config editor surfaces as an error toast rather than an escaping exception; and `auth login`, `data migrate` and `skill install` pick up the `0600` config mode they were missing.
- `t`'s dashboard meaning is now cursor-aware: on an issue row in the issue list it still opens that issue's ticket transcript, unchanged (#330); everywhere else on a repo — the rail, the repo detail panel, and the PR pane — it now opens the new per-repo chat instead, one derived key with two honest labels decided by `viewActions.ts`'s `bodyVerbs`. `ReviewView`'s chat-draft list gains `s`/`e`/`r` (submit / edit / cycle route), derived and appended after the existing `all`/`none`/`file`/`discard` so those four keys stay pinned.
...
- The outbox no longer writes a corrupted PR URL into a finished ticket (#348). When an offline-queued PR op replays and `gh pr create` reports that the PR already exists, the outbox recovers the URL with `gh pr view … --jq .url` — and took that stdout on faith. If `gh` exited 0 with empty output or a bare `null`, the op checkpointed an empty PR URL, wrote it into the done ticket's `## Result` block (where the dependency sweep reads it to unblock dependents, rewriting it on every subsequent flush), and posted an "Opened " comment with nothing after it on the linked issue. An empty or `null` recovery now fails the op retryably with the URL still unset: nothing is written back, no comment or label flip is attempted, and the next flush retries the create from scratch instead of resuming from a bogus checkpoint.
- The shipped config templates no longer earn a deprecation warning on day one (#377). `examples/config.json` and `examples/config.hosted.json` — what a fresh install copies from, and part of the npm package — still opened with the deprecated `vaultRoot`/`juncoSubdir` queue-root override and set the equally deprecated `git.worktreeRoot`, so a copy produced a startup deprecation and a `junco doctor` finding immediately; the canonical `dataDir` key never appeared, and neither did the `github`, `botAccount`, `planSets`, `skills`, `updateCheck` or `worker.applyFallbackToAgent` sections. Both templates now lead with `"dataDir": "~/.junco"`, drop the three legacy path keys, and carry every missing section at its schema default, with a test that fails if they drift from the schema again.

### Security

- A ticket's `## Verification` bash blocks now run under the same sandbox backend and policy as the agent's own `bash` (#335). They previously spawned as a bare `/bin/bash -c` with only the environment scrub applied — no OS confinement, unrestricted network, the whole filesystem — while executing whatever the agent had just written into the worktree: a `package.json` script, a Makefile target, a `conftest.py`. Tokens were scrubbed from the environment, but the bot's `gh` credential, `~/.ssh`, and every other checkout on the host live on disk and stayed reachable. Blocks now get network per `sandbox.network` and the per-ticket `network:` opt-in, writes limited to the worktree, its scratch dir, the linked worktree's git metadata and `sandbox.extraAllowWrite`, and `TMPDIR` redirected into scratch. The sandbox is resolved lazily (a block-less ticket never pays for the probe) and once per ticket, and a resolver failure fails every block closed rather than running it unconfined. The new live-reload lever `verify.sandboxed` (default `true`) opts out, and is inert when `sandbox.enabled` is `false`. See [Configuration § Verification sandbox](docs/configuration.md#verification-sandbox).
- The sandbox's builtin read deny-list now covers the standard credential files (#336). The read side is "the whole host minus a short list", and that list stopped at `~/.ssh`, `~/.aws`, `~/.config/gh`, `~/.gnupg` and `~/.pi` — so on every ticket the agent could read `~/.npmrc`, `~/.git-credentials`, `~/.netrc`, `~/.docker/config.json`, `~/.kube`, `~/.config/gcloud`, `~/.cargo/credentials.toml`, `~/.pypirc`, `~/.gem/credentials` and `~/.claude`. Where the credential is a single file inside a directory the agent legitimately needs — cargo's `bin/` and `registry/`, docker's `cli-plugins/` — only the file is denied, not the directory, and the deny is rendered as a file mask or a subtree deny according to what the path actually is, so it means the same thing on all three backends. The list is not operator-removable, as before.
- junco now scans the diff it is about to push and refuses the push on a hit (#337). `sandbox.network: deny` governs the **agent's** tool calls; it is never consulted when junco itself runs `git add -A` over the worktree and pushes under the bot identity — so the push was an egress channel no sandbox rule saw, and content copied into a source file left the host through junco's own privileged push. The added lines of `<base>..HEAD` are now matched against a deliberately narrow set of high-confidence shapes: PEM private-key headers, GitHub token prefixes, `AKIA…` access-key IDs, `sk-ant-`, Stripe live keys, Slack `xox…-` tokens, npm tokens, and credentials embedded in a URL — the shape a copied `.netrc`, `.npmrc` or git remote takes. On a hit the push is refused, the worktree is preserved, and the ticket fails with a note naming `path:line` and the rule that matched — never the matched content, which is not logged, stored, or written to the ticket record. `pr.secretScan` (default `true`) opts out a repository that legitimately vendors credential-shaped fixtures. See [Configuration § Pre-push secret scan](docs/configuration.md#pre-push-secret-scan).
- `junco ask`, `junco audit` and `junco investigate` can no longer write to the checkout they read (#346). All three run the agent in the operator's real checkout, and the sandbox policy made that cwd a writable root unconditionally — the read-only promise rested entirely on the tool allowlist. A locally dispatched ticket's `tools:` is used verbatim, so one naming `bash` or `write` silently turned a read-only run into full write access to the live checkout, `.git` included: two layers that should be independent were one. The three flows now build their policy with scratch as the only writable root, so the in-process path jail and both OS backends refuse writes into the checkout whatever the allowlist says. Reads of the checkout are unaffected, and the PR flow and the critic keep their existing policy.
- An apply ticket's patch series can no longer reach `git am` with a path junco never inspected (#338, #339). The containment check — traversal paths, absolute paths, binary hunks — was enforced only by plan-lint's `patch_paths_sane` rule, which runs only while `planLint.enabled` and `planLint.blockOnError` are both on, both live-editable levers an operator may plausibly turn off for the style rules. With lint off, a series naming `../escaped.txt` reached `git am`, and git's own rejection rolled into the escalation ladder, which by default handed the series to an agent as its "reviewed" specification. `applyPatchSeries` now runs the same checks itself, unconditionally, before any side effect — no temp file, no transcript frames, no git call — and the ticket fails with `apply failed: refused before git am: …` regardless of the fallback toggle. Separately, the series parser recognised only the plain `diff --git a/… b/…` header, while git C-quotes a path whenever it contains a quote, backslash, tab or non-ASCII byte under the default `core.quotePath`: such a hunk's path never entered the file list the containment check reads, so one ordinary hunk was enough to keep the series "well-formed" while a quoted-path hunk rode along unexamined. Any `diff --git` line that is not the plain form now rejects the whole series, and `rename from/to` / `copy from/to` paths are extracted explicitly, since git applies those names over the header line's.
- Both OS sandbox backends tightened what a sandboxed agent can reach beyond the filesystem. On macOS the Seatbelt profile opened with `(deny default)` and then granted unrestricted Mach name lookup, so the agent's bash could reach every service on the host — securityd (the login keychain that `git credential-osxkeychain` reads), fseventsd, the TCC-mediated daemons; `security list-keychains` exited 0 from inside the sandbox. That blanket grant is replaced by an enumerated allowlist, established empirically by running node, git, npm, python3, go, ruby, perl, bun, swiftc, cc and make under the profile and adding names until they passed: exactly one turns out to be needed, the directory-services lookup without which `git commit` fails and `whoami` prints a raw id (#340). On Linux the agent's bash now gets its own IPC and UTS namespaces and a fresh session — `--new-session`, which closes the TIOCSTI terminal-injection escape outright — on top of the pid and network isolation it already had (#345).
- `sandbox.requireBackend` (boolean, default `false`, live-reload) makes the default `sandbox.backend: "auto"` fail closed on a failed probe (#344). Until now a host with no `bwrap`, or a kernel that refuses unprivileged user namespaces, silently degraded the session to the `none` backend: the environment scrub and the in-process path jail survived, but agent `bash` ran with no OS confinement while `junco status` looked healthy and only a log warning and a `junco doctor` ⚠ said otherwise. With the lever on, an unavailable auto-selected backend refuses the session outright and `junco doctor` reports `✗ sandbox` and exits 1; both refusal messages name the lever, since it is the opt-out rather than `sandbox.enabled`. It is inert for an explicitly named backend and for `backend: "none"`, which already failed closed. See [Configuration § Sandbox backend](docs/configuration.md#sandbox-backend).
- The GitHub bridge now vouches a sub-issue's **parent** body against the trigger label, not just the child's (#342). When a labeled issue is a sub-issue, the bridge injects the parent's title and body into the planner prompt as background context — but only the child was checked by the edited-after-label guard, and the parent is a different issue with a different author that could be edited at any time after the label vouched the child. The same `lastEditedAt` check now runs at the parent's own repository and number (sub-issues may live across repos); a parent that postdates the label, cannot be verified, or is missing its coordinates has its context dropped with a `warn` log line. A failed parent check never blocks the child, which was vouched on its own.
- The final issue comment junco posts at finalize — the Q&A answer, or the PR-link excerpt — is sanitized before it is published (#341). It carried the model's text verbatim, and junco recovers plans by looking for a `<!-- junco:plan -->` marker in its own bot-authored comments, so a prompt-injected agent could plant a forged marker on the very issue the plan flow reads back. The text now goes through the same sanitizer the parked audit and investigate drafts already use — HTML comments and control characters stripped, capped at the comment limit — and an answer that sanitizes to nothing falls back to the existing `_(no answer text)_` placeholder.
- The data tree is created private: `0700` on the root and every subdirectory, `0600` on `config.json` and on transcript `.jsonl` files (#343). On a shared or multi-account host, every local user could previously read the config — which may hold a literal `model.apiKey` — and every transcript under the data root, which is the full agent event stream including verbatim contents of private-repo files; all of it was created under the default umask. The mode rides the temp file through the atomic rename, so a rewrite tightens a loose file too. Because `mkdir` never re-modes a tree that already exists, `junco doctor` gains a `data tree modes` check over the config file, the data root and the transcripts directory that warns — never fails — with the exact `chmod` to run; it is skipped on Windows, where the bits are meaningless.
